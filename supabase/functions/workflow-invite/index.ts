...
async function handleInvite(req: Request, user: AppUser): Promise<Response> {
  try {
    ...
    if (!body.workflow_id || !body.user_email || !body.role) {
      return createErrorResponse('Missing required fields: workflow_id, user_email, role');
    }
    ...
    const isOwner = workflow.user_id === user.id;
    ...
    if (!isOwner) {
      ...
      if (!collaborator || collaborator.role !== 'owner') {
        return createErrorResponse('Unauthorized: Only workflow owners can invite collaborators', 403);
      }
    }
    ...
    const { data: collaboratorId, error: addError } = await supabase.rpc('add_collaborator', {
      ...
      p_resource_id: body.workflow_id,
    ...
    // await sendInviteEmail(inviteeData.email, workflow.name, user.full_name);
